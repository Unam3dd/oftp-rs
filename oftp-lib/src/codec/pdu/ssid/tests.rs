...
mod setter_tests {
    use super::*;
    use crate::codec::pdu::ssid::constant::{BUF_SIZE_MAX, BUF_SIZE_MIN, CREDIT_MAX};

    #[test]
    fn set_level_and_mode_chain() {
        let mut ssid = Ssid::default();
        ssid.set_level(ProtocolLevel::Rev14)
            .set_mode(SsidMode::SendOnly);
        assert_eq!(ssid.level, ProtocolLevel::Rev14);
        assert_eq!(ssid.mode, SsidMode::SendOnly);
    }

    #[test]
    fn set_buffer_size_bounds() {
        let mut ssid = Ssid::default();
        ssid.set_buffer_size(BUF_SIZE_MIN).unwrap();
        assert_eq!(ssid.buffer_size, BUF_SIZE_MIN);
        ssid.set_buffer_size(BUF_SIZE_MAX).unwrap();
        assert_eq!(ssid.buffer_size, BUF_SIZE_MAX);
        assert!(matches!(
            ssid.set_buffer_size(BUF_SIZE_MIN - 1),
            Err(SsidFieldError::BufferSizeOutOfRange)
        ));
        assert!(matches!(
            ssid.set_buffer_size(BUF_SIZE_MAX + 1),
            Err(SsidFieldError::BufferSizeOutOfRange)
        ));
    }

    #[test]
    fn set_credit_bounds() {
        let mut ssid = Ssid::default();
        ssid.set_credit(CREDIT_MAX).unwrap();
        assert_eq!(ssid.credit, CREDIT_MAX);
        assert!(matches!(
            ssid.set_credit(CREDIT_MAX + 1),
            Err(SsidFieldError::CreditOutOfRange)
        ));
    }

    #[test]
    fn set_password_and_flags() {
        let mut ssid = Ssid::default();
        ssid.set_password("SECRET").unwrap();
        assert_eq!(&ssid.password[..6], b"SECRET");
        ssid.set_compression(true)
            .set_restart(true)
            .set_authentication(true)
            .set_special_logic(false);
        assert!(ssid.compression);
        assert!(ssid.restart);
        assert!(ssid.auth);
        assert!(!ssid.special_logic);
    }

    #[test]
    fn set_password_too_long() {
        let mut ssid = Ssid::default();
        assert!(matches!(
            ssid.set_password("123456789"),
            Err(SsidFieldError::PasswordTooLong)
        ));
    }

    #[test]
    fn setters_survive_encode_decode() {
        let mut ssid = Ssid::default();
        ssid.set_code("TEST01").unwrap();
        ssid.set_level(ProtocolLevel::Rev20)
            .set_mode(SsidMode::ReceiveOnly);
        ssid.set_buffer_size(8192).unwrap();
        ssid.set_credit(75).unwrap();
        ssid.set_compression(true).set_restart(false);

        let buf = ssid.encode().unwrap();
        let mut decoded = Ssid::default();
        decoded.decode(&buf).unwrap();
        assert_eq!(decoded.mode, SsidMode::ReceiveOnly);
        assert_eq!(decoded.buffer_size, 8192);
        assert_eq!(decoded.credit, 75);

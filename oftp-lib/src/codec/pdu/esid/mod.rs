...
impl Esid {
    ...
    pub fn decode(&mut self, buf: &[u8]) -> Result<(), EsidError> {
        ...
        self.reason = parse_numeric_field(&buf[ESID_REASON_OFFSET..ESID_TEXT_LEN_OFFSET])? as u8;
    ...
    pub fn encode(&self) -> Result<Vec<u8>, EsidError> {
        ...
        v.push(ESIDCMD);
        v.extend_from_slice(&encode_numeric_field(self.reason as u64, ESID_REASON_LEN)?);
